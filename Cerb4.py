...
class API(object):
  ...
  def __init__(self, access_key, secret_key, base='/cerb5/index.php/webapi'):
  ...
  def __connect(self, verb, url, payload=''):
    ...
    if query == '':
      fullUrl   = '%s/%s.xml' % (self.base, path)
    else:
      fullUrl   = '%s/%s.xml?%s' % (self.base, path, query)
    ...
    headers['Cerb4-Auth']  = '%s:%s' % (self.accessKey, b64Hash)
